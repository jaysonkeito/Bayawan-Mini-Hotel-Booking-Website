<?php
  ...
  if(isset($_POST['upd_contacts']))
  {
      ...
      // iframe contains HTML so we handle it separately — not through filteration()
      $iframe = $_POST['iframe'] ?? '';

      echo update(
          "UPDATE `contact_details` SET `address`=?,`gmap`=?,`pn1`=?,`pn2`=?,`email`=?,`fb`=?,`insta`=?,`tw`=?,`iframe`=? WHERE `sr_no`=?",
