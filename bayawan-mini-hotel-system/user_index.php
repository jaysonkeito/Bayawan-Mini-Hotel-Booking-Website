...
<body class="bg-light">
        ...
        <div class="col-lg-12 bg-white shadow p-4 rounded">
            ...
            <div class="row align-items-end">
                ...
                <select class="form-select shadow-none" name="adult">
                    ...
                    for($i = 1; $i <= ($guests_res['max_adult'] ?? 0); $i++){
                ...
                <select class="form-select shadow-none" name="children">
                    <?php 
                    echo "<option value='0'>0</option>";
                    for($i = 1; $i <= ($guests_res['max_children'] ?? 0); $i++){
                        echo "<option value='$i'>$i</option>";
                    }
                    ?>
                </select>
        ...
        <?php 
            ...
            if(mysqli_num_rows($thumb_q) > 0){
                ...
                if(mysqli_num_rows($first_img_q) > 0){
                    $first_img = mysqli_fetch_assoc($first_img_q);
                    $room_thumb = ROOMS_IMG_PATH . $first_img['image'];
                }
            }
            ...
            // Rating
            $rating_res = mysqli_query($conn, "SELECT AVG(rating) AS `avg_rating` FROM `rating_review`
    ...
    <?php
        if(isset($_SESSION['google_error'])){
            $err = $_SESSION['google_error'];
            unset($_SESSION['google_error']);
            echo "<script>setTimeout(() => alert('error', '$err'), 100);</script>";
        }

        if(isset($_SESSION['google_new'])){
            unset($_SESSION['google_new']);
            echo "<script>setTimeout(() => alert('success', 'Welcome! Please update your profile with your phone number and address.'), 100);</script>";
        }
    ?>
    ...
    <?php
        ...
        $query = select("SELECT * FROM `user_cred` WHERE `email`=? AND `remember_token`=? LIMIT 1",
